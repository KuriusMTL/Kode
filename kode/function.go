 ...
 */
func CreateFunction(name string, argumentsTemplate []Argument, variables map[string](*Variable), returnType string, parent *Function, code string) Function {
...
func (scope *Function) ExtractFunctionArgs(queue *Queue, depth int64) ([]*Variable, error) {
	...
	for !queue.IsEmpty() {
		...
		if (nextToken.(string) == "(" || nextToken.(string) == "[") && !isInString {
			...
			parameter, err := EvaluateExpression(scope, tempVariable, depth)
			if err != nil {
				return nil, err
			}
 ...
 * @return error - The error, if any.
*/
func (scope *Function) Run(args []*Variable, vars map[string](*Variable), depth int64) (*Variable, int, error) {
	...
	if (*scope).VariableExists("_MAX_RECURSION") && EvaluateType((*(*scope).GetVariable("_MAX_RECURSION")).Value) == "int" {
		if depth > (*(*scope).GetVariable("_MAX_RECURSION")).Value.(int64) {
			return nil, 0, errors.New("Maximum recursion depth reached.")
		}
	} else {
	...
	for currentLine := 0; currentLine < len(lines); currentLine++ {
		...
		for !tokens.IsEmpty() {
			...
			case "val", "int", "float", "string", "bool":
				...
				if err != nil {
					return nil, 0, errors.New(err.Error() + " on line " + strconv.Itoa(currentLine+1))
				}
				...
				if !nameProvided {
					return NullVariable(), 0, errors.New("Error: Missing variable name on line " + strconv.Itoa(currentLine+1) + ".")
				}

				// Check if the variable name is valid
				if !HasValidVariableName(name.(string)) {
					return nil, 0, errors.New("Error: Invalid variable name on line " + strconv.Itoa(currentLine+1) + ". The name must be alphanumeric and start with a letter.")
				}

				// Check if the variable name is already in use in the current scope
				if (*scope).VariableExists(name.(string)) {
					return NullVariable(), 0, errors.New("Error: Variable was already declared on line " + strconv.Itoa(currentLine+1) + ".")
				}
				...
				if !assign || equal.(string) != "=" {
					return NullVariable(), 0, errors.New("Error: Missing variable assignment \"=\" on line " + strconv.Itoa(currentLine+1) + ".")
				}
				...
				if value == "" {
					return NullVariable(), 0, errors.New("Error: Missing variable value on line " + strconv.Itoa(currentLine+1) + ".")
				}

				// Create the variable and evaluate the value
				evaluatedValue, err := EvaluateExpression(scope, value, depth)
				if err != nil {
					return NullVariable(), 0, errors.New(err.Error() + " on line " + strconv.Itoa(currentLine+1) + ".")
				}
				...
				if command.(string) != "val" && evaluatedValue.Type != command.(string) {
					...
					if !isArrayType(command.(string)) || evaluatedValue.Type != "val[]" || len(evaluatedValue.Value.([]Variable)) != 0 {
						return NullVariable(), 0, errors.New("Error: Invalid variable type on line " + strconv.Itoa(currentLine+1) + ". The type of the variable must be " + command.(string) + ".")
					} else {
			...
			case "if":
				...
				for _, conditionBlock := range conditionBlocks {
					...
					if conditionBlock.Condition == "else" {
						...
						ifCondition := CreateFunction("if", []Argument{}, (*scope).Variables, "val", scope, conditionBlock.Code)
						returnValue, toReturn, err := ifCondition.Run([]*Variable{}, map[string]*Variable{}, depth)
						...
						// Evaluate the condition
						evaluatedCondition, err := EvaluateExpression(scope, conditionBlock.Condition, depth)
						if err != nil {
							return NullVariable(), 0, errors.New(err.Error() + " on line " + strconv.Itoa(conditionBlock.ConditionIndex+1) + ".")
						}

						// Check the type of the evaluated condition
						// If it is not a boolean, return an error
						if evaluatedCondition.Type != "bool" {
							return NullVariable(), 0, errors.New("Error: Invalid condition on line " + strconv.Itoa(conditionBlock.ConditionIndex+1) + ". The condition must be a boolean.")
						}
			...
			case "func":
				...
				if !nameProvided {
					return NullVariable(), 0, errors.New("Error: Missing function name at decleration on line " + strconv.Itoa(currentLine+1) + ".")
				}

				// Check if the function name is valid
				// Again, they act like variables
				if !HasValidVariableName(name.(string)) {
					return NullVariable(), 0, errors.New("Error: Invalid function name on line " + strconv.Itoa(currentLine+1) + ". The name must be alphanumeric and start with a letter.")
				}
				...
				if (*scope).VariableExists(name.(string)) {
					return NullVariable(), 0, errors.New("Error: Function was already declared by another variable on line " + strconv.Itoa(currentLine+1) + ".")
				}
				...
				if !charProvided || char.(string) != "(" {
					return NullVariable(), 0, errors.New("Error: Missing opening parenthesis for the parameters on line " + strconv.Itoa(currentLine+1) + ".")
				}
				...
				for !tokens.IsEmpty() {
					...
					if !tokenProvided {
						return NullVariable(), 0, errors.New("Error: Missing closing parenthesis for the parameters on line " + strconv.Itoa(currentLine+1) + ".")
					}
					...
					if token.(string) == ")" {
						...
						if token.(string) != "val" && token.(string) != "int" && token.(string) != "float" && token.(string) != "bool" && token.(string) != "string" {
							return NullVariable(), 0, errors.New("Error: Invalid parameter type \"" + token.(string) + "\" on line " + strconv.Itoa(currentLine+1) + ".")
						}
						...
						if !parameterNameProvided {
							return NullVariable(), 0, errors.New("Error: Missing parameter name on line " + strconv.Itoa(currentLine+1) + ".")
						}

						// Check if the parameter name is valid
						// If it is not, return an error
						if !HasValidVariableName(parameterName.(string)) {
							return NullVariable(), 0, errors.New("Error: Invalid parameter name \"" + parameterName.(string) + "\" on line " + strconv.Itoa(currentLine+1) + ". The name must be alphanumeric and start with a letter.")
						}
						...
						if token.(string) == "," {
							...
							return NullVariable(), 0, errors.New("Error: Invalid function syntax \"" + token.(string) + "\" on line " + strconv.Itoa(currentLine+1) + ".")
						}
				...
				if !returnTypeProvided {
					...
					return NullVariable(), 0, errors.New("Error: Invalid return type \"" + returnType.(string) + "\" on line " + strconv.Itoa(currentLine+1) + ".")
				}
				...
				if !funcEnded {
					return NullVariable(), 0, errors.New("Error: Missing end for function \"" + name.(string) + "\" on line " + strconv.Itoa(functionStart+1) + ".")
				}
				...
				function := CreateVariable(CreateFunction(name.(string), parameters, make(map[string]*Variable), returnType.(string), scope, functionCode))
			...
			case "return":
				...
				if expression != "" {
					// Evaluate the expression
					value, err := EvaluateExpression(scope, expression, depth)
					if err != nil {
						return NullVariable(), 0, err
					}
				...
				if returnValue == (Variable{}) {
					...
					return NullVariable(), 1, errors.New("Error: Invalid return type \"" + returnValue.Type + "\" on line " + strconv.Itoa(currentLine+1) + ".")
				}
			...
			case "for":
				...
				evaluatedCondition, err := EvaluateExpression(scope, loopBlock.Condition, depth)
				if err != nil {
					return NullVariable(), 0, errors.New(err.Error() + " on line " + strconv.Itoa(currentLine+1) + ".")
				}

				if evaluatedCondition.Type != "bool" {
					return NullVariable(), 0, errors.New("Error: Invalid condition type \"" + evaluatedCondition.Type + "\" on line " + strconv.Itoa(currentLine+1) + ".")
				}

				for evaluatedCondition.Value.(bool) {

					forLoop := CreateFunction("for", []Argument{}, (*scope).Variables, "val", scope, loopBlock.Code)
					returnValue, toReturn, err := forLoop.Run([]*Variable{}, map[string]*Variable{}, depth)
				...
				if (*scope).VariableExists(command.(string)) {
					...
					for validPeek && peeked.(string) == "[" {
						...
						if !isArrayType((*variable).Type) {
							return NullVariable(), 0, errors.New("Error: '" + peeked.(string) + "' cannot access that index because it might not be an array on line " + strconv.Itoa(currentLine+1) + ".")
						}

						tokens.Pop()
						indexArray, err := (*scope).ExtractArrayValues(&tokens, depth)
						if err != nil {
							return NullVariable(), 0, errors.New(err.Error() + " on line " + strconv.Itoa(currentLine+1) + ".")
						}
						if len(indexArray) != 1 || indexArray[0].Type != "int" {
							return NullVariable(), 0, errors.New("Error: Invalid array index on line " + strconv.Itoa(currentLine+1) + ".")
						}
						size := int64(len((*variable).Value.([]Variable)))
						if size == 0 {
							return NullVariable(), 0, errors.New("Error: Array is empty on line " + strconv.Itoa(currentLine+1) + ".")
						}
					...
					if !assign || (equal.(string) != "=" && equal.(string) != ":=") {
						...
						_, err := EvaluateExpression(scope, output, depth)
						...
						if ((*variable).Type != evaluatedValue.Type) && equal.(string) != ":=" {
							...
							if !isArrayType((*variable).Type) && !isArrayType(evaluatedValue.Type) && strings.ReplaceAll((*variable).Type, "[]", "") != "val" {
								return NullVariable(), 0, errors.New("Error: Variable type mismatch on line " + strconv.Itoa(currentLine+1) + ". Expected type " + (*scope).Variables[command.(string)].Type + " but got type " + evaluatedValue.Type + ".")
							} else {
					...
					args, err := (*scope).ExtractFunctionArgs(&tokens, depth)
					...
					_, err = RunBuiltIn(command.(string), args)
					if err != nil {
						return NullVariable(), 0, err
					}
					...
					// Command is unknown
					return NullVariable(), 0, errors.New("Error: Unknown command \"" + command.(string) + "\" on line " + strconv.Itoa(currentLine+1) + ".")
				}
